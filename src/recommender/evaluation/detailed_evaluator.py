...
class DetailedEvaluator:
    """
    Evaluator that returns detailed recommendations with movie titles for each user.
    Useful for LLM reranking and manual inspection.
    """
    def __init__(self, k=10, device="auto", batch_size=128, item_batch_size=512,
                 max_test_users=None, test_user_seed=42):
        self.k = k
    ...
    def evaluate(self, model, data, train_data=None, val_data=None, return_details=True):
        """
        Evaluate model and optionally return detailed recommendations.

        Returns:
            If return_details=False: (hr, ndcg, recall)
            If return_details=True: (hr, ndcg, recall, recommendations_df)
        """
        model.eval()
        model.to(self.device)

        # Load movie metadata if returning details
        if return_details:
        ...
        recommendations = [] if return_details else None
        ...
        with torch.no_grad():
            for batch_start in range(0, len(test_users), self.batch_size):
                ...
                for idx, u in enumerate(batch_users):
                    ...
                    if return_details:
                        for rank, (item_id, score) in enumerate(zip(topk_items, topk_scores_np), 1):
                            movie_info = self._get_movie_info(item_id)
        ...
        if return_details:
            ...
            return hr, ndcg, recall, recommendations_df
        else:
    ...
    def _load_movie_metadata(self):
        """Load movie metadata from MovieLens dataset."""
        if self.movies_df is None:
            movies, _, _ = download_data("movielens", force=False, env="dev")
            self.movies_df = movies
            logger.info(f"Loaded {len(self.movies_df)} movies for metadata")

    def _get_movie_info(self, item_id: int) -> dict:
        """Get movie title and genres for an item ID."""
        if self.movies_df is None:
            return {'title': f'Item {item_id}', 'genres': 'Unknown'}

        # item_id is the index in the factorized movies
        # We need to map it back to movieId
        try:
            movie_row = self.movies_df.iloc[item_id]
            return {

...
def test_gat_encode_interface(tiny_hetero_graph):
    ...
    assert z_dict["user"].shape == (g['user'].num_nodes, 64 * 2)  # num_heads * hidden_dim
    assert z_dict["item"].shape == (g['item'].num_nodes, 64 * 2)
...
def test_gat_different_dims(tiny_hetero_graph, hetero_graph_100_200):
    ...
    assert z_dict1["user"].shape == (g1['user'].num_nodes, 64 * 2)
    assert z_dict1["item"].shape == (g1['item'].num_nodes, 64 * 2)
    ...
    assert z_dict2["user"].shape == (g2['user'].num_nodes, 128 * 2)
    assert z_dict2["item"].shape == (g2['item'].num_nodes, 128 * 2)
...
def test_gat_variable_layers(hetero_graph_50_50):
    ...
    assert z_dict1["user"].shape == (g['user'].num_nodes, 64 * 2)
    assert z_dict1["item"].shape == (g['item'].num_nodes, 64 * 2)
    ...
    assert z_dict3["user"].shape == (g['user'].num_nodes, 64 * 2)
    assert z_dict3["item"].shape == (g['item'].num_nodes, 64 * 2)
...
def test_gat_forward_interface(tiny_hetero_graph):
    ...
    assert "user" in z_dict
    assert "item" in z_dict
    assert z_dict["user"].shape == (g['user'].num_nodes, 64 * 2)
    assert z_dict["item"].shape == (g['item'].num_nodes, 64 * 2)
...
def test_gat_encode_shape(tiny_hetero_graph):
    ...
    assert out['user'].shape == (g['user'].num_nodes, 64 * 2)
    assert out['item'].shape == (g['item'].num_nodes, 64 * 2)
...
def test_gat_forward_pass_different_graph_sizes(tiny_hetero_graph, large_hetero_graph):
    ...
    assert out2['user'].shape == (g2['user'].num_nodes, 64 * 2)
    assert out2['item'].shape == (g2['item'].num_nodes, 64 * 2)
